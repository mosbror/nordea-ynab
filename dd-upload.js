var ddup = {
  ...
  queue : function (files) {
    ...
    for (let f of files) {
      ...
      ddup.hstat.innerHTML += `<div>Added ${f.name} to queue</div>`;
      // ADD TO QUEUE
      ddup.upqueue.push(f); 
    }
    // GO!
  ...
  go : function () { if (!ddup.uplock && ddup.upqueue.length!=0) {
    ...
    // OPTIONAL - SHOW UPLOAD STATUS
    ddup.hstat.innerHTML += `<div>Upload started</div>`;
    ...
    xhr.onload = function () {
      // OPTIONAL - SHOW UPLOAD STATUS
      ddup.hstat.innerHTML += `<div>${this.response}</div>`;
      // NEXT BETTER PLAYER!
      ddup.uplock = false;
      ddup.go();
    };
    xhr.onerror = function(evt){
      // OPTIONAL - SHOW UPLOAD STATUS
      ddup.hstat.innerHTML += `<div>AJAX ERROR</div>`;
      // NEXT BETTER PLAYER!
      ddup.uplock = false;
      ddup.go();
    };
